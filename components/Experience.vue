<script setup></script>
<template>
    <section
        class="education-experience-section bg-gray-50 text-indigo-900 py-16"
    >
        <div class="container mx-auto px-6 md:px-12 my-10">
            <!-- Section Title -->
            <div class="text-center mb-12">
                <h2 class="text-4xl md:text-5xl font-bold inline-block">
                    Education & Experience
                    <span
                        class="block w-28 h-1 bg-yellow-400 mt-2 mx-auto"
                    ></span>
                </h2>
            </div>
            <!-- Columns -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <!-- Column 1: Education -->
                <div class="space-y-10">
                    <strong>Education</strong>
                    <div>
                        <p class="text-sm text-gray-700">2023 - Present</p>
                        <h3 class="text-xl font-semibold">
                            Master of Library & Information Science
                        </h3>
                        <p class="text-base text-gray-700">
                            San Jose State University
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-700">2020 - 2022</p>
                        <h3 class="text-xl font-semibold">
                            Bachelor of Arts, Art History
                        </h3>
                        <p class="text-base text-gray-700">
                            University of California, Los Angeles
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-700">2016 - 2020</p>
                        <h3 class="text-xl font-semibold">
                            Associate in Arts, Art History & Studio Art
                        </h3>
                        <p class="text-base text-gray-700">
                            Pasadena City College
                        </p>
                    </div>
                </div>

                <!-- Column 2: Professional Experience -->

                <div class="space-y-10">
                    <strong> Experience</strong>

                    <div>
                        <p class="text-sm text-gray-700">May 2023 - Present</p>
                        <h3 class="text-xl font-semibold">
                            Curatorial Assistant
                        </h3>
                        <p class="text-base text-gray-700">
                            Los Angeles Municipal Art Gallery
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-700">
                            July 2024 - March 2025
                        </p>
                        <h3 class="text-xl font-semibold">
                            Assistant Registrar
                        </h3>
                        <p class="text-base text-gray-700">
                            Academy Museum of Motion Pictures
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-700">
                            July 2022 - May 2024
                        </p>
                        <h3 class="text-xl font-semibold">Library Pager</h3>
                        <p class="text-base text-gray-700">
                            Autry Museum of the American West
                        </p>
                    </div>
                </div>

                <!-- Column 3: Related Experience & In Progress -->
                <div class="space-y-10">
                    <strong>Professional Development</strong>
                    <div>
                        <p class="text-sm text-gray-700">
                            June 2025 - August 2025
                        </p>
                        <h3 class="text-xl font-semibold">Archives Intern</h3>
                        <p class="text-base text-gray-700">
                            Ellsworth Kelly Foundation
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-700">
                            July 2024 - March 2025
                        </p>
                        <h3 class="text-xl font-semibold">
                            Conservation Technician
                        </h3>
                        <p class="text-base text-gray-700">
                            The Civic Art Division
                        </p>
                    </div>
                    <div>
                        <p class="text-sm text-gray-700">2025 - Present</p>
                        <h3 class="text-xl font-semibold">Conservation</h3>
                        <p class="text-base text-gray-700">
                            Dominguez Scultpure Restoration
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>
